...
while True:
    ...
    if fen == "n":       
        ...
        fen = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/7R"
        board = read_Fen(fen, board) if not c else read_Fen(fen[::-1], board)  
        # print(board)
        break
    else:
        print("invalid input")
...
def piece_board(board, allocated_board):
    ...
    piece_map = {
        ...
        4:  ("white", 2, 3, white_Knight),
        5:  ("black", 2, 3, black_Knight),
        8:  ("white", 3, 3, white_Bishop),
        9:  ("black", 3, 3, black_Bishop),
        16: ("white", 4, 5, white_Rook),
        17: ("black", 4, 5, black_Rook),
        32: ("white", 5, 9, white_Queen),
        33: ("black", 5, 9, black_Queen),
        64: ("white", 6, 30, white_King),
        65: ("black", 6, 30, black_King),
    }
    ...
    for x in board:
        ...
        if piece in piece_map:
            ...
            allocated_board[x] = Piece(pos_board_pieces[x][0], pos_board_pieces[x][1], color, piece, value1, value2, image, x)
    ...
    if c:
        for x in range(8):
            for y in range(8):
                ...
                if board[x + y * 8] != 0:
                    pos_board_pieces[x + y * 8] = ((700 - x * 100 , 700 - y * 100))
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if clicked:
                ...
                if board[r_posx + r_posy * 8] != 0:
                    ...
                    board_pieces[63 -r_posx + 63 -r_posy * 8].selected = False if board_pieces[63 - r_posx +  63 - r_posy * 8].selected else True
                    draw_board()
                    render_pieces(board_pieces)
                    print(piecevalue)
    pygame.display.flip()
